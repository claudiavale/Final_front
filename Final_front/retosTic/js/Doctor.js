...
function cargarDatosDoctor(id) {
    $.ajax({
        dataType: 'json',
        url:"http://129.151.105.50:8080/api/Doctor/"+id,
        type: 'GET',
    ...
    if($("#name2").val().length == 0 || $("#department").val().length == 0 || $("#year").val().length == 0 || $("#description2").val().length == 0){
       alert("Todos los campos son obligatorios")
            ...
            $.ajax({
                ...
                url:"http://129.151.105.50/api/Doctor/save",
                data: dataToSend,
                datatype: 'json',
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://129.151.105.50:8080/api/Doctor/"+idElemento,
            type: 'DELETE',
            contentType: "application/JSON",
            success: function (response) {
                console.log(response);
...
function actualizar(idElemento) {
    ...
    if($("#name2").val().length == 0 || $("#department").val().length == 0 || $("#year").val().length == 0 || $("#description2").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://129.151.105.50:8080/api/Doctor/update",
            type: "PUT",
